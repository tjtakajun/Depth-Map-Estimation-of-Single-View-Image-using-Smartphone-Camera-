...
public class PointCloudPathController : MonoBehaviour
{
    ...
    private ManualPointCloud _manualPointCloud;
    ...
    public void NewPointCloud()
    {
        ...
        if (PointCloud != null)
        {
            Debug.Log("PointCloud object created successfully 2.");
        }
        else
        {
            Debug.LogError("Failed to create PointCloud object 2.");
        }
        // ポイントクラウドオブジェクトのスケールを調整
        float pointCloudScale = 0.030f; // 例として 0.001 をスケールとして使用
        PointCloud.transform.localScale = new Vector3(pointCloudScale, pointCloudScale, pointCloudScale);
